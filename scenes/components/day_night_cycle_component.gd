...
@export var initial_day: int = 1:
	set(id):
		initial_day = id
		DayAndNightCycleManager.intial_day = id
		DayAndNightCycleManager.set_initial_time()

@export var initial_hour: int = 12:
	set(ih):
		initial_hour = ih
		DayAndNightCycleManager.intial_hour = ih
		DayAndNightCycleManager.set_initial_time()
	
@export var initial_minute: int = 30:
	set(im):
		initial_minute = im
		DayAndNightCycleManager.intial_minute = im
		DayAndNightCycleManager.set_initial_time()

@export var day_night_gradient_texture: GradientTexture1D

func _ready() -> void:
	DayAndNightCycleManager.intial_day = initial_day
	DayAndNightCycleManager.intial_hour = initial_hour
	DayAndNightCycleManager.intial_minute = initial_minute
	DayAndNightCycleManager.set_initial_time()
